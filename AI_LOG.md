...
### Follow-up before 19A.2: the flaky assertion was corrected

That assertion was then fixed on its own, ahead of implementation work, so a known-unreliable test would not be sitting under 19A.2. The change is confined to `tests/integration/stub-intelligence-isolation.test.ts`; no production code, schema or Milestone 18 behaviour was touched, and the Milestone 18 entry above was left as written.

The actual tool responses were inspected before rewriting anything. `explain_score` returns an empty envelope for the stub company — `data` null, `citations` empty, and a warning naming the company — while `search_targets` returns the stub identity with every score-derived field null. The substring scan was replaced by assertions on those named fields: the stub match must have null `normalizedScore`, `coverage`, `lowerBound`, `upperBound`, `recommendation`, `path`, `thesis` and `whyNow`, and `hasResearch` false; the explanation must have null `data` and no citations. That is strictly stronger than the original, which only checked `normalizedScore`. A contrast assertion on the real company in the same tool call keeps the test from passing merely because the reader returned nulls for everyone.

Evidence: the focused file passed twelve consecutive runs, and three more after formatting. Twelve clean runs is by itself weak evidence against a roughly 6% failure rate, so the mechanism was also checked directly — reconstructing the envelope with a slug and an `asOf` that both contain `99` and no leaked score, the old assertion fails and the new ones pass. A mutation check pointed the stub assertions at the real company and the test failed with `normalizedScore: 87` against the expected null, confirming the new assertions still detect intelligence rather than passing vacuously. The full integration suite then reported 57 passed with 12 failures, all of them the intended Milestone 19A.1 RED tests. Lint, typecheck, 266 unit tests and Prettier on the changed file all passed.
